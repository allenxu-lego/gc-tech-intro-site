...
import { DIFY_EXECUTION_STEPS } from './flowchartData';
 ...
 *
 * Displays an overlay above the e-aliyun-jenkins-dify edge showing AI workflow deployment steps.
 * Auto-scrolls through items with fade animations when triggered by clicking jenkins-for-dify node.
 * Click to pause/resume. If at last item, click to restart from beginning.
 */
export function DifyExecStepsLayer() {
  const {
  ...
  const currentItem = DIFY_EXECUTION_STEPS[difyExecStepsCurrentIndex];
  const Icon = currentItem?.icon;

  // Check if at last item
  const isAtLastItem = difyExecStepsCurrentIndex >= DIFY_EXECUTION_STEPS.length - 1;

  // Auto-scroll animation effect
  useEffect(() => {
    // Don't run if not visible, paused, or at last item
    if (!difyExecStepsVisible || isPaused || isAtLastItem) {
      return;
    }
    ...
    timerRef.current = setTimeout(() => {
      setDifyExecStepsCurrentIndex(difyExecStepsCurrentIndex + 1);
  ...
  }, [difyExecStepsVisible, isPaused, isAtLastItem, difyExecStepsCurrentIndex, setDifyExecStepsCurrentIndex]);
  ...
  // Position calculation: midpoint between jenkins-for-dify (550, 320) and dify (300, 320)
  const position = {
    x: (975) / 2 - LAYER.WIDTH / 2, // = 375
  ...
  return (
    <g
      ...
      role="button"
      aria-label={`AI Workflow Deployment Steps: ${currentItem?.label || ''}${isPaused ? ' (Paused)' : ''}${isAtLastItem ? ' - Click to restart' : ''}`}
      aria-live="polite"
      aria-atomic="true"
    ...
    >
      ...
      {/* Progress indicator dots - ice blue theme (4 dots for Dify steps) */}
      <g transform={`translate(${LAYER.WIDTH / 2 - 18}, ${LAYER.HEIGHT - 8})`}>
        {DIFY_EXECUTION_STEPS.map((step, index) => (
          <circle
            key={step.id}
            cx={index * 12}
            cy={0}
            r={2}
            fill={index <= difyExecStepsCurrentIndex ? '#38bdf8' : '#bae6fd'}
          />
        ))}
      </g>
    </g>
